...
// Shared IP-geolocation engine for the homepage globe's point layers
// (provider nodes and Nostr relays) — server-side only (uses node:dns).
// Extracted from app/api/providers/globe-points/route.ts so a second
// points-building route (relays) doesn't duplicate the lookup/cache logic
// and doesn't independently hammer the same external geo APIs.

const IPWHO_BASE_URL = "https://ipwho.is";
const IP_API_BASE_URL = "http://ip-api.com/json";
const GEO_TIMEOUT_MS = 8000;
const GEO_CACHE_TTL_MS = 1000 * 60 * 60;
export const GEO_LOOKUP_CONCURRENCY = 8;
...
// Geolocates a hostname or IP with a 1hr in-memory cache and in-flight
// dedup, trying ip-api.com then ipwho.is directly on the host, then falling
// back to a DNS-resolved public IP through the same two providers.
export async function fetchGeoForHost(host: string): Promise<{ lat: number; lng: number } | null> {
