...
export async function fetchModels(): Promise<void> {
  try {
    ...
    const activeProviders = data.providers.filter(
      ({ health }) => health.status_code === 200 && health.json?.models && health.json.models.length > 0
    );
    ...
    activeProviders.forEach(({ provider, health }) => {
      ...
      if (health.json?.models) {
        ...
        const providerModels = health.json.models.map(rawModel => modelMap.get(rawModel.id)!);
...
export function getPrimaryProviderForModel(modelId: string): Provider | undefined {
  const providersForModel = modelProvidersMap.get(modelId);
  return providersForModel && providersForModel.length > 0 ? providersForModel[0] : undefined;
}
...
export function getUsdPricingShort(model: Model): string {
  ...
  const inStr = `$${inPerMTokens.toFixed(inPerMTokens >= 0.1 ? 2 : 2)}/M input tokens`;
  const outStr = `$${outPerMTokens.toFixed(outPerMTokens >= 0.1 ? 2 : 2)}/M output tokens`;
  return `${inStr}  |  ${outStr}`;
}
...
export function getProviderById(id: string): Provider | undefined {
  return providers.find(provider => provider.id === id || provider.d_tag === id);
}
