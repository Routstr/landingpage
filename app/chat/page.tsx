          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="bg-black/40 border border-white/10 rounded-lg p-2">
              <div className="flex items-center">
                <span className="text-sm font-bold mr-1">{balance}</span>
                <span className="text-xs text-gray-300">sats</span>
              </div>
            </div>
            ...
            <div className="flex space-x-2">
              ...
              <Drawer open={isModelDrawerOpen} onOpenChange={setIsModelDrawerOpen}>
                ...
                <DrawerContent className="bg-black border-t border-white/10 p-4">
                  <DrawerHeader>
                    <DrawerTitle className="text-white">Select AI Model</DrawerTitle>
                  </DrawerHeader>
              ...
              <Drawer open={isConversationDrawerOpen} onOpenChange={setIsConversationDrawerOpen}>
                <DrawerTrigger asChild>
                  <button className="bg-black/40 border border-white/10 rounded-lg p-2 text-xs">
                    History
                  </button>
                </DrawerTrigger>
                <DrawerContent className="bg-black border-t border-white/10 p-4">
                  <DrawerHeader className="flex items-center justify-between">
                    <DrawerTitle className="text-white">Conversations</DrawerTitle>
                    <button
                      onClick={() => {
                        createNewConversation();
                  ...
                  <div className="space-y-2 max-h-[50vh] overflow-y-auto mt-2">
                    {conversations.length === 0 ? (
                      <p className="text-xs text-gray-300 text-center py-2">No saved conversations</p>
                    ) : (
              ...
              <div className="min-h-full p-6 space-y-4">
                {messages.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-gray-400 pt-24">
                    ...
                    <p className="text-sm text-center">Send a message to start chatting with {selectedModel?.name || 'AI'}</p>
                  </div>
                ) : (
                  messages.map((message, index) => (
                    ...
                    >
                      <div className={`flex flex-col ${message.role === 'user' ? 'items-end' : 'items-start'} max-w-[85%]`}>
                        ...
                        {message.role === 'user' && editingMessageIndex !== index && (
                          <div className={`mt-1 ${isMobile ? 'opacity-50' : 'opacity-0 group-hover:opacity-100'} transition-opacity duration-200`}>
                            <button
